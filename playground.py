...
import numpy as np
import boto3
from fuzzup.whitelists import EBLocalNames
...
d_ = EBLocalNames()
# Apply whitelists
out = apply_whitelists([c, m, n], clusters, score_cutoff=98, scorer=WRatio)
...
def get_news_data(content_ids, cols=["article_id", "title", "lead", "body"]):
    ...
    logger.info(
        f"Querying data for {n_content_ids} news articles with columnns {cols} from data lake..."
    )
    ...
    logger.info(
        f"Extracted news data successfully for {len(df)}/{n_content_ids} content ids"
    )
    # formatting
...
get_news_data([9150838], cols=["article_id", "title", "subtitle", "body_text"])
