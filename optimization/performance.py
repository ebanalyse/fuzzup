...
def flatten_whitelist(wl_clusters) -> List:
    ...
    if len(wl_clusters['eblocal_name'])> 0 or len(wl_clusters['municipality'])> 0:
        for whitelist in wl_clusters:
            for matches in wl_clusters[whitelist]:
                match = matches['matches']
                for mapping in matches ['mappings']:
                    if whitelist == 'eblocal_name':
                        if (match, mapping['eblocal_id']) not in match_city:
                            match_city.append((match, mapping['eblocal_id']))
                    if whitelist == 'municipality':
                        if match not in match_municipality:
                          match_municipality.append(match)
    return match_city
 
def _predict(preds: List[Dict],
             #cutoff_fuzz,
             cutoff_wl,
             #scorer_fuzz,
             scorer_wl,
    ...
    fuzzy_preds = fuzzy_cluster_bygroup(
        preds, cutoff=98, scorer=token_ratio
    )
    ...
    whitelist_matches = apply_whitelists(WHITELISTS, fuzzy_preds, scorer =FUZZUP_SCORERS[scorer_wl], score_cutoff = cutoff_wl, aggregate_cluster=True)
...
def process_dataset(train: pd.DataFrame, #cutoff_fuzz,
             cutoff_wl,
         #    scorer_fuzz,
             scorer_wl,
             wgt_body,
