...
def _predict(preds:List[Dict], cutoff,scorer) -> Dict:
...
def process_dataset(train: pd.DataFrame, cutoff, scorer) -> List[Dict]:
    ...
    with tqdm(total=train['content_id'].nunique()) as pbar:
        for article in np.unique(train['content_id']):
            ...
            preds_dict[article].update(_predict(preds_list, cutoff,scorer))
    return preds_dict  
