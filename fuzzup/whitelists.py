...
def get_municipalities():
    ...
    whitelist = {" ".join([x.get('navn'), "Kommune"]): {'municipality_code': x.get('kode'), 'match_name': " ".join([x.get('navn'), "Kommune"]), "type": "id_municipality"} for x in data}
    return whitelist
...
def get_eblocal_names():
    ...
    # remove "hits"
    eblocals.pop(0)
    out = {x['eblocal_name'] : {'eblocal_id': x['eblocal_id'], 'match_name':x['eblocal_name'], "type": "id_eblocal"} for x in eblocals}
    return out
